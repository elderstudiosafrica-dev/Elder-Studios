...
export default function Team() {
  return (
    <section className="relative bg-[#101a20] px-6 py-32 md:px-16 md:py-44">
      <Reveal className="flex flex-col items-center">
        <Crest className="h-20 w-20 text-mist" />
        <h2 className="mt-5 font-brand text-5xl uppercase tracking-[0.04em] text-mist md:text-[4.8rem]">
          THE TEAM
        </h2>
      </Reveal>
      ...
      <div className="mx-auto mt-28 grid max-w-[1360px] grid-cols-2 gap-x-4 gap-y-14 md:grid-cols-4 md:gap-x-0">
        {team.map((member, i) => (
          <Reveal key={member.name} delay={i * 0.1} className="flex flex-col items-center text-center">
            ...
            />
            <h3 className="mt-7 font-brand text-3xl text-mist md:text-[2.25rem]">
              {member.name}
            </h3>
            <p className="mt-2 max-w-[290px] font-display text-xl uppercase leading-[1.05] text-mist-dim md:text-2xl">
              {member.roles.join(" · ")}
            </p>
          </Reveal>
        ))}
